...
\hi{Alternate Logic-gate Representation}
  \hii{Conversion Procedure}
    ...
    \end{enumerate}
    \begin{figure}[H]
      \includegraphics[width=15cm]{c03/alternate-gates.jpg}
  ...
  \hiiBEGIN{Logic-Symbol Interpretation}
    \hiii{Active-HIGH - Active-LOW}
    \begin{itemize}
      \item When an input or output line on a logic circuit symbol \tb{does
        not} have bubble on it, that line is said to be \tb{active-HIGH}.
      \item When an input or output line on a logic circuit symbol \tb{does}
        have bubble on it, that line is said to be \tb{active-LOW}.
    \end{itemize}

    \hiii{Interpretation}
      \par To interpret the logic-gate operation:
      \begin{itemize}
        \item \tb{Step 1}: Note which logic state is the active state for the input.
        \item \tb{Step 2}: Note which logic state is the active state for the output.
        \item \tb{Step 3}: Based on the type of gate, there are two cases:
          \begin{itemize}
    ...
    \par \tb{The output goes LOW only when \ti{all} of the inputs are HIGH}.
  \hiiEND
