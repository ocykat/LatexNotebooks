...
\hi{NAND Gate Latch}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics{c05/simplified-block-nand-latch.JPG}
...
\hi{Clock Signals and Clocked Flip-flops}
  ...
  \hii{Clocked Flip-flops}
    ...
    \begin{itemize}
      \item A FF with \tb{a small triangle} on its CLK input to indicate
        that this input is activated only when a \textbf{PGT} occurs;
        no other part of the input pulse will have an effect
        on the CLK input.
      \item A FF with \tb{a small triangle with a bubble} on its CLK input
